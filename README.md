...
Suite frontend : 51 tests Vitest (client API avec rafraîchissement JWT, stockage de tokens, `proxy.ts` de protection de routes, création de signalement et upload de photo, construction des requêtes filtrées, formatage de dates), plus intégration réelle testée contre un backend Django lancé en parallèle avec un vrai worker Celery actif — voir `frontend/README.md`.
